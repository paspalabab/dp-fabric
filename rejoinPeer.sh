...
do
   ...
   if [ ${each_org_config_path} = "org1" ]; then
       ...
       docker exec fabric-cli /bin/sh -c "cd ${FABRIC_CLI_WORK_HOME}/${each_org_config_path}; pwd; ./createAndSignNewOrgConfig.sh --upconfigpath ${FABRIC_CLI_WORK_HOME}/configtx/update_in_envelope.pb"
   fi
done
...
do
   ...
   docker exec fabric-cli /bin/sh -c "cd ${FABRIC_CLI_WORK_HOME}/${each_org_config_path}; pwd; ./query.sh; ./fetchCurChanConfig.sh ${FABRIC_CLI_WORK_HOME}/configtx"
done